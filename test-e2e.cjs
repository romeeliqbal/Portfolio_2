...
async function run() {
  ...
  const sections = ['hero', 'summary', 'about', 'experience', 'technologies', 'projects', 'education', 'certifications', 'resume', 'contact'];
  ...
  if (techBtn) {
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'desktop-tech-modal.png') });
  ...
  if (menuBtn) {
    ...
    await page.screenshot({ path: path.join(screenshotDir, 'mobile-menu-open.png') });
  }
