...
function EduPulseDemo() {
  ...
  const courses = [
    { code: 'CS-301', name: 'Data Structures & Algorithms', enrolled: 48, cap: 50, status: '96% CAPACITY' },
    { code: 'SE-204', name: 'Database Systems & Architecture', enrolled: 43, cap: 45, status: '95% CAPACITY' },
    { code: 'SE-102', name: 'Web Engineering Principles', enrolled: 58, cap: 60, status: '97% CAPACITY' },
    { code: 'CS-402', name: 'Operating Systems & Security', enrolled: 32, cap: 50, status: '64% CAPACITY' }
  ];

  const filteredCourses = filter === 'HIGH' 
    ...
    <div className="space-y-4 border border-[#222222] bg-[#0A0A0A] p-5 font-mono text-xs">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3 bg-[#111111] border border-[#222222]">
          <span className="text-[9px] text-[#666666] block uppercase">Total Enrolled</span>
          <span className="text-base font-bold text-white">1,420</span>
        </div>
        <div className="p-3 bg-[#111111] border border-[#222222]">
          <span className="text-[9px] text-[#666666] block uppercase">Avg Attendance</span>
          <span className="text-base font-bold text-white">94.2%</span>
        </div>
        <div className="p-3 bg-[#111111] border border-[#222222]">
          <span className="text-[9px] text-[#666666] block uppercase">Courses Tracked</span>
          <span className="text-base font-bold text-white">18</span>
        </div>
        <div className="p-3 bg-[#111111] border border-[#222222]">
          <span className="text-[9px] text-[#666666] block uppercase">Schema Status</span>
      ...
      <div className="space-y-2 pt-2">
        <span className="text-[10px] text-[#888888] uppercase block">Course Enrollment Capacity</span>
        {filteredCourses.map((c) => {
          const pct = Math.round((c.enrolled / c.cap) * 100);
          return (
            <div key={c.code} className="p-2.5 bg-[#121212] border border-[#1E1E1E]">
              <div className="flex items-center justify-between text-[11px] mb-1.5">
                <span className="text-white font-medium">{c.code} &bull; {c.name}</span>
                <span className="text-[#AAAAAA]">{c.enrolled}/{c.cap} ({pct}%)</span>
              </div>
    ...
    <div className="border border-[#222222] bg-[#0A0A0A] p-5 font-mono text-xs space-y-4">
      ...
      {running ? (
        <div className="py-8 text-center space-y-2">
          ...
          <span className="text-[10px] text-[#777777] block uppercase">Running sandbox verification...</span>
        </div>
      ) : (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-[10px] pt-1">
        ...
        <div className="p-2 border border-[#1E1E1E] bg-[#111111] col-span-2 sm:col-span-1">
          ...
          <span className="text-white font-medium">Verified Clean Rollback</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="border border-[#222222] bg-[#0A0A0A] p-5 font-mono text-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-[#1E1E1E]">
        ...
        <span className="text-[10px] text-[#666666] uppercase">SAMPLE INPUT SELECTOR</span>
      </div>
        ...
        {samples.map((s, idx) => (
          <button
            ...
            className={`px-3 py-1.5 border text-[10px] uppercase transition-colors ${
              selectedPrompt === idx ? 'border-white text-white bg-[#1A1A1A]' : 'border-[#262626] text-[#777777] hover:text-white'
            }`}
          >
            Sample {idx + 1}
          </button>
      ...
      <div className="p-3 bg-[#111111] border border-[#1E1E1E]">
        <span className="text-[9px] text-[#666666] uppercase block mb-1">Student Feedback Text</span>
        <p className="text-white text-xs leading-relaxed font-sans">{current.text}</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[11px]">
        <div className="p-3 bg-[#111111] border border-[#1E1E1E]">
          <span className="text-[9px] text-[#666666] uppercase block mb-1">Polarity Score</span>
          <span className="text-white font-medium">{current.polarity}</span>
        </div>
        <div className="p-3 bg-[#111111] border border-[#1E1E1E]">
          <span className="text-[9px] text-[#666666] uppercase block mb-1">Extracted Keywords</span>
          <div className="flex flex-wrap gap-1.5 mt-1">
            {current.keywords.map(kw => (
              <span key={kw} className="text-[9px] px-1.5 py-0.5 bg-[#1E1E1E] text-[#CCCCCC] border border-[#333333]">
                {kw}
              </span>
            ))}
          </div>
      ...
      <div className="p-3 bg-[#141414] border border-[#2A2A2A]">
        <span className="text-[9px] text-[#888888] uppercase block mb-1">Generated Administrative Action</span>
...
function MadamsBoutiqueDemo({ liveUrl }) {
  return (
    <div className="border border-[#222222] bg-[#0A0A0A] p-5 font-mono text-xs space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3 bg-[#111111] border border-[#1E1E1E]">
          <span className="text-[9px] text-[#666666] block uppercase">Pages Engineered</span>
          <span className="text-base font-bold text-white">7 Pages</span>
        </div>
        <div className="p-3 bg-[#111111] border border-[#1E1E1E]">
          <span className="text-[9px] text-[#666666] block uppercase">Runtime JS Bundle</span>
          <span className="text-base font-bold text-white">14.2 KB</span>
        </div>
        <div className="p-3 bg-[#111111] border border-[#1E1E1E]">
          <span className="text-[9px] text-[#666666] block uppercase">Lighthouse Perf</span>
          <span className="text-base font-bold text-white">100 / 100</span>
        </div>
        <div className="p-3 bg-[#111111] border border-[#1E1E1E]">
          <span className="text-[9px] text-[#666666] block uppercase">Layout Shift (CLS)</span>
          <span className="text-base font-bold text-white">0.000</span>
        </div>
      </div>

      <div className="p-3 bg-[#111111] border border-[#1E1E1E] text-xs">
        <span className="text-[9px] text-[#666666] uppercase block mb-1.5">Page Architecture Manifest</span>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[10px] text-[#AAAAAA]">
          ...
          <div className="border-l border-[#333333] pl-2">02. Curated Catalog</div>
          <div className="border-l border-[#333333] pl-2">03. Product Inspector</div>
          <div className="border-l border-[#333333] pl-2">04. Slide Cart Drawer</div>
          <div className="border-l border-[#333333] pl-2">05. Editorial Lookbook</div>
          <div className="border-l border-[#333333] pl-2">06. Brand Narrative</div>
          <div className="border-l border-[#333333] pl-2">07. Responsive Checkout</div>
        </div>
      </div>
    </div>
  );
            ...
            <div className="p-6 sm:p-8 bg-[#0E0E0E] border border-[#2A2A2A] space-y-6">
              ...
              {activeTab === 'demo' ? (
                <div>
                  ...
                  {project.id === '04' && <MadamsBoutiqueDemo liveUrl={project.liveUrl} />}
                </div>
              ) : (
